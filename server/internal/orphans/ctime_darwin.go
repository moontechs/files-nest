...
// ctime returns the inode status-change time for info. On Darwin this is
// read from the st_ctimespec field of the syscall.Stat_t. Unlike
// mtime/atime, ctime cannot be set by os.Chtimes, so it is the one
// timestamp the orphan scan's age guard can trust even though
// filestore.MoveFile now writes client-controlled mtime/atime values (see
// docs/adr/0006).
