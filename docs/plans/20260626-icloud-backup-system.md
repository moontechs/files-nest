...
- [ ] `go mod init` with module name, add dependencies: `dgraph-io/badger/v4`, `chi` router
- [ ] open BadgerDB in `internal/store/store.go`: `Open(path string) (*Store, error)`
- [ ] write tests: Open creates DB directory, second Open on same path succeeds
...
- [ ] implement `uploadbackend.Client` with `CreateUpload(metadata string) (backendID string, err error)` — calls upload backend `POST /files` with `Upload-Defer-Length: 1`
- [ ] implement `POST /uploads` handler: parse `{ localIdentifier, metadata, creationDate, filename, bundleId }`, call `uploadbackend.Client.CreateUpload`, store record in BadgerDB (status: uploading), return `{ id, upload_url: "/uploads/<id>/data" }`
- [ ] handle conflict: if record already exists, return the existing record's current status to the Mac app regardless of what that status is — let the client decide: `uploading` → resume; `complete` → skip; `deleted` → skip (only re-upload if user explicitly re-syncs); `backend_lost` → treat as new upload (call DELETE /uploads/:id first, then POST again)
...
- [ ] write httptest tests for all three handlers
...
- [ ] add `uploadbackend.Client` methods: `GetOffset(backendID string) (int64, error)`, `ForwardPatch(backendID string, r *http.Request) (*http.Response, error)`, `Terminate(backendID string) error`
- [ ] implement `HEAD /uploads/:id/data`: look up backend_id from BadgerDB, proxy HEAD to upload backend; if backend returns 404, call `UpdateStatus(id, "backend_lost")` and return `409 Conflict` with body `{"error":"backend_lost"}` — never return the backend 404 raw
- [ ] implement `PATCH /uploads/:id/data`: proxy PATCH body + TUS headers to upload backend; if backend returns 404, apply same `backend_lost` transition and return 409
- [ ] write httptest tests: correct offset returned; chunk forwarded with correct headers; 404 on unknown record id; backend 404 transitions to backend_lost and returns 409
...
- [ ] implement `DELETE /uploads/:id` handler: send TUS Termination DELETE to upload backend (treat backend 404 as success — it may already be gone), then call `UpdateStatus(deleted)`, return 204
- [ ] write tests: PATCH complete succeeds; PATCH with status=deleted returns 400; collision suffix applied when dest exists; MoveFile failure leaves status unchanged; DELETE ignores backend 404
...
- [ ] read config from env: `BACKUP_USER`, `BACKUP_PASS`, `UPLOAD_BACKEND_URL`, `STORAGE_PATH`, `PORT`
...
- [ ] Dockerfile: multi-stage Go build (pure Go, no CGO needed — BadgerDB is pure Go)
- [ ] docker-compose: server + upload backend services + Caddy reverse proxy
...
- [ ] create new macOS app in Xcode: SwiftUI, menu bar only (LSUIElement = YES in Info.plist)
- [ ] add `com.apple.security.personal-information.photos-library` to entitlements
- [ ] add `NSPhotoLibraryUsageDescription` to Info.plist
- [ ] set up NSStatusItem with a cloud icon in `MenuBarController`
- [ ] attach empty SwiftUI popover on click
- [ ] verify app shows in menu bar and popover opens/closes
- [ ] no automated test for this task — manual verification only
...
- [ ] define `AssetProtocol`: localIdentifier, creationDate, mediaType, mediaSubtypes, pixelWidth, pixelHeight, duration, isFavorite, isHidden, burstIdentifier, sourceType, location
- [ ] define `AssetLibraryProtocol`: `requestAuthorization() async -> Bool`, `fetchAssets(from:to:) -> [AssetProtocol]`, `resources(for:) -> [AssetResourceProtocol]`
- [ ] define `AssetResourceProtocol`: type (photo/video/pairedVideo), uniformTypeIdentifier, originalFilename
- [ ] define `AssetResourceManagerProtocol`: `requestData(for:options:dataReceivedHandler:completionHandler:)`
- [ ] implement `PhotoLibrary` conforming to `AssetLibraryProtocol` using real PHAsset/PHAssetResource
- [ ] implement `PHAsset`, `PHAssetResource`, `PHAssetResourceManager` conformance extensions in a separate file
- [ ] create `FakeAsset`, `FakeAssetLibrary`, `FakeAssetResourceManager` in test target for use in Tasks 11–14
- [ ] no automated tests for PhotoLibrary itself (requires real Photos library) — manual verification only
...
- [ ] implement `KeychainStore` with `save(serverURL:user:password:)` and `load() -> Credentials?`
- [ ] use `Security.framework` `SecItemAdd` / `SecItemCopyMatching` / `SecItemUpdate`
- [ ] build `SettingsView`: server URL field, username, password (SecureField), "Test Connection" button
- [ ] Test Connection: HEAD request to server with credentials, show success/failure inline
- [ ] on save: write to Keychain, dismiss settings
- [ ] show settings automatically on first launch (no credentials in Keychain)
- [ ] write unit tests for KeychainStore save + load roundtrip
...
- [ ] implement `ServerClient` with `URLSession` + Basic Auth header injected on every request
- [ ] `createUpload(localIdentifier:metadata:creationDate:filename:bundleId:) async throws -> UploadRecord` — POST /uploads; caller inspects `record.status` to decide whether to upload, resume, or skip
- [ ] `listUploads(from:to:status:limit:cursor:) async throws -> UploadPage` (`UploadPage` has `items` + `nextCursor`; cursor is the opaque composite value from the server)
- [ ] `getOffset(id:) async throws -> Int64` — HEAD /uploads/:id/data, reads Upload-Offset header; throws `BackendLostError` on 409 with error=backend_lost
- [ ] `uploadChunk(id:data:offset:isLast:) async throws -> Int64` — PATCH /uploads/:id/data; sends `Upload-Offset`, `Content-Type: application/offset+octet-stream`, `Upload-Complete: 1` on last chunk; throws `BackendLostError` on 409
- [ ] `updateStatus(id:status:) async throws` — PATCH /uploads/:id/status; only valid value is `"complete"`
- [ ] `deleteUpload(id:) async throws` — DELETE /uploads/:id
- [ ] `uploadURL` is constructed by the client as `<baseURL>/uploads/<id>/data` — never stored as a relative path; no trailing-slash ambiguity
- [ ] define `UploadRecord` Codable struct matching server response; include `status` field typed as `UploadStatus` enum
- [ ] define `BackendLostError` so callers can handle re-registration without string-matching
- [ ] write tests using `URLProtocol` stub: correct URL and Auth header; correct decoding; cursor pagination; 409 backend_lost throws BackendLostError
- [ ] create `FakeServerClient` in `ServerClientFakes.swift` implementing the same protocol as `ServerClient` — used by Tasks 14 and 15 instead of `URLProtocol` stubs
...
- [ ] implement `MetadataSerializer.serialize(_ asset: AssetProtocol, resource: AssetResourceProtocol) -> [String: String]` using protocol types (not PHAsset directly)
- [ ] include all fields: localIdentifier, dates, location, mediaType, dimensions, duration, isFavorite, isHidden, burstIdentifier, sourceType, filename, bundleId, resourceType
- [ ] encode metadata dict as TUS-compatible base64 key-value pairs
- [ ] write unit tests using `FakeAsset` from Task 9
...
- [ ] implement `AssetUploader.upload(asset:resource:uploadId:startOffset:) async throws` taking `AssetProtocol` and `AssetResourceProtocol`
- [ ] inject `AssetResourceManagerProtocol` and `ServerClient` — never reference `PHAssetResourceManager` directly; no TUSClient wrapper
- [ ] bridge `requestData` callback API to async/await using `AsyncThrowingStream`: the `dataReceivedHandler` closure appends `Data` chunks to the stream; `completionHandler` finishes or errors it — this avoids holding more than one in-flight callback result at a time
- [ ] back-pressure: do not call `requestData` in fire-and-forget fashion; await each `uploadChunk` before the stream can deliver the next callback — use a bounded `AsyncChannel` (capacity 1) or explicit semaphore so iCloud cannot outpace TUS uploads; for a 7GB file at 8MB chunks that means at most 1 chunk buffered, not 875
- [ ] accumulate incoming data in a mutable buffer; flush as TUS PATCH exactly when buffer reaches chunk size (configurable, default 8MB); remainder is the final chunk
- [ ] on last chunk (stream exhausted), set `isLast: true` to send `Upload-Complete: 1`
- [ ] if `startOffset > 0`, discard initial iCloud data up to `startOffset` bytes before buffering — log clearly that iCloud restarted from byte 0 (resume asymmetry, expected)
- [ ] propagate errors from both iCloud download side and TUS upload side; if `ServerClient` throws `BackendLostError`, surface it immediately so `SyncCoordinator` can re-register
- [ ] write tests using `FakeAssetResourceManager` + `FakeServerClient`: verify chunk sequencing; verify exactly one chunk buffered at a time under back-pressure; verify offset skip on resume; verify `BackendLostError` propagated
...
- [ ] implement `SyncCoordinator.sync(range: DateRange?) async throws` (nil = full sync)
- [ ] inject `AssetLibraryProtocol`, `ServerClient`, `AssetUploader` — no `TUSClient` (eliminated)
- [ ] fetch PHAssets via `AssetLibraryProtocol`; page through server `GET /uploads` using cursor until `nextCursor` is empty; treat cursor as opaque — pass it back verbatim
- [ ] build upload queue: assets in library not in server uploads (status = complete); include both resources of Live Photo pairs; treat the pair as one unit — skip both or upload both
- [ ] build delete queue: server uploads not in current library (scoped to range); delete Live Photo pairs together via two sequential `deleteUpload` calls
- [ ] process resume: server records with `status=uploading` → `getOffset` → resume `AssetUploader.upload(startOffset:)` from that offset; if `getOffset` throws `BackendLostError`, call `createUpload` to re-register (resets to fresh upload, startOffset=0)
- [ ] handle `BackendLostError` during `uploadChunk`: call `deleteUpload` to clean up the lost record, then `createUpload` to re-register, then restart upload from offset 0
- [ ] run uploads sequentially (1 at a time); call `updateStatus(complete)` after each
- [ ] run deletes after uploads complete
- [ ] sync cooldown: persist a `lastSyncStarted: Date` in `UserDefaults`; on launch, if a sync was in progress when the app last quit (stored state = `syncing`), resume from the first incomplete item in the upload queue rather than rebuilding from scratch — avoids re-diffing an entire 10k library on each relaunch after a crash mid-sync
- [ ] expose `@Published var progress: SyncProgress` (total, completed, currentFileName)
- [ ] write tests with `FakeAssetLibrary` + `FakeServerClient` + `FakeAssetUploader`: upload queue correct; delete queue correct; Live Photo pairs treated as unit; resume uses HEAD offset; BackendLostError triggers re-registration; cursor pagination exhausted; cooldown resumes from correct position
...
- [ ] build `ContentView` with SwiftUI:
...
- [ ] buttons disabled while sync is running
- [ ] wire buttons to `SyncCoordinator.sync(range:)`
- [ ] show error alert if sync fails
- [ ] manual UI verification — no automated test for this task

---

### Task 17: Verify acceptance criteria

- [ ] full sync: fetches all library assets (paginated), uploads missing ones, deletes orphans on server
- [ ] range sync: correctly scoped to date range, deletions checked within range
- [ ] resume: interrupted upload resumes from last offset on next sync (TUS HEAD, not DB)
- [ ] 7GB file: streams without OOM, no temp file on disk
- [ ] Live Photo: both JPEG and MOV resources uploaded, linked by bundle_id
- [ ] credentials: saved to Keychain, never to UserDefaults or plist
- [ ] first launch: settings shown automatically if no credentials
- [ ] TLS: server reachable only over HTTPS, Basic Auth credentials not sent in cleartext
- [ ] run full test suite: `swift test` in `files-nest-mac/`
- [ ] run server tests: `go test ./...` in `server/`
...
- [ ] server README: setup, env vars (`BACKUP_USER`, `BACKUP_PASS`, `STORAGE_PATH`, `DOMAIN`, `PORT`), docker-compose usage, API reference
- [ ] app README: build requirements (Xcode, entitlements), how to configure server URL
