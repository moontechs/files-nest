...
- [x] run `make mutation-test` (or the equivalent `gremlins` invocation per the `server/Makefile` target) scoped to `internal/filestore`, `internal/store`, `internal/orphans` — the three packages this plan touches
  - Ran `gremlins unleash --timeout-coefficient=40 --workers=4` per package (auth: `gremlins` path + `go` toolchain at `/home/worker/go/bin`). Pre-warmed the build cache and raised the timeout coefficient — the default `timeout-coefficient: 10` produced large TIMED-OUT counts on the cold test/compile cache that are non-deterministic and inconclusive; with a warm cache + coefficient 40 every mutant resolved to KILLED/LIVED. Results: `filestore` 44 mutants (1 in-scope survivor), `store` 112 mutants (3 survivors, all pre-existing), `orphans` 11 mutants (0 survivors).
- [x] review the report for surviving mutants specifically in the new/changed code from this plan: `parseCreationDate`, `isSaneCreationDate`, the `Chtimes` branch in `MoveFile`, `PlanDestResult.DateUsed` plumbing, `CompletionIntent.CreationDate` round-trip, both `ctime()` platform functions, `Candidate.CTime` population in `Scan`, and `FilterMinAge`'s `CTime`-based comparison
  - In-scope survivors found: exactly one — `mover.go:374 CONDITIONALS_NEGATION`, the `err != nil` branch in `applyCreationTimestamp`'s `os.Chtimes` error path (Task 4 new code). All other survivors are in pre-existing code this plan never touched and are out of scope: `filestore` `SafePathSegment` `r < 32` (`mover.go:603`) and `len(out) > maxPathSegmentLen` (`mover.go:616`, added in pre-plan commit 8ef7739); `store` `index.go:79` `len(date) >= 10` and `uploads.go:644/698` `lastSlash < 0` / `limit > 1000`. `orphans` had zero survivors. All listed plan functions (`parseCreationDate`, `isSaneCreationDate`, `DateUsed`, `CreationDate` round-trip, both `ctime()` funcs, `Scan`'s `CTime` population, `FilterMinAge`) were fully KILLED.
- [x] for each surviving mutant, strengthen the corresponding test's assertions (per ADR 0002: fix by strengthening tests, not by loosening gremlins' config or ignoring survivors) — pay particular attention to the boundary conditions this plan introduces: the `isSaneCreationDate` min/max edges, the `FilterMinAge` `>=` boundary re-verified against `CTime` instead of `ModTime`, and the `Chtimes` error-vs-success branch
  - Added `TestApplyCreationTimestampLogsChtimesFailure` to `mover_internal_test.go`: redirects `log` output to a buffer, calls `applyCreationTimestamp` with a non-existent destination + a sane date so `os.Chtimes` deterministically fails, and asserts the failure is logged and the call returns without panicking. This closes the `Chtimes` error-vs-success gap that Task 9 had flagged as uncovered-by-inspection; it kills the `mover.go:374` `err != nil` → `err == nil` mutant. The `isSaneCreationDate` min/max edges and `FilterMinAge` `CTime` `>=` boundary were already strongly asserted (all mutants KILLED, no strengthening needed).
- [x] re-run `make mutation-test` after fixes — must show no new survivors in the touched packages before task 11
  - Re-ran `gremlins` on all three touched packages. The in-scope `mover.go:374` survivor is now KILLED; remaining LIVED are only the pre-existing `SafePathSegment`/`index.go`/`uploads.go` survivors in code this plan never modified (no NEW survivors from this plan's code in any touched package). Note: the unmodified `make mutation-test` target runs `gremlins unleash ./internal/...` (covering untouched `api`/`uploadbackend` too) and enforces `threshold-efficacy: 100`; that whole-repo run cannot be green because of pre-existing survivors in untouched packages, so the per-touched-package scoped run is the meaningful gate here.
- [x] run full test suite again: `cd server && go test ./...` — must still pass after test strengthening
  - Passed: `api`, `filestore`, `orphans`, `store`, `uploadbackend` all ok; `go vet ./...` clean.
